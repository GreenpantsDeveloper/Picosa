...
- `/workspace` shows the current repository to work on.
...
Most importantly:
- **Anything saved to `~/.pi/` is TEMPORARY** and does not persist outside of the current session. Write to the mounted repository at `/workspace` instead.
